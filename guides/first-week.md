...
- **Vault check.** Ask: "what's in the vault so far?" People and projects you've talked about should be showing up as entities. If someone you've mentioned repeatedly isn't there, say "make a vault entry for them." The index of everything, `vault/index.md`, is regenerated by a command rather than kept up by hand, so it can't quietly fall behind what's on disk.
...
The session-start hook that loads your files didn't run. Say: "Read the operating manual in my HQ and follow the startup protocol," which fixes this session. (The manual is `CLAUDE.md` or `AGENTS.md`, depending on which agent set the HQ up; either name gets you there, since the one your agent doesn't use is a stub pointing at the other.) If it recurs, say "the session-start hook isn't firing, check that it's registered and executable." On Codex, the usual cause is the one-time hook approval: it only offers that on a project you've told it to trust. In Cowork, expect it: those sessions run on Anthropic's servers and may never run your HQ's hooks, so the one-line fix above is the routine there rather than a repair.
