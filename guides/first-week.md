...
The session-start hook that loads your files didn't run. Say: "Read CLAUDE.md and follow the startup protocol," which fixes this session. If it recurs, say "the session-start hook isn't firing, check that it's registered and executable." On Codex, the usual cause is the one-time hook approval: it only offers that on a project you've told it to trust.
